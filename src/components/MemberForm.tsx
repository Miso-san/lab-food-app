...
function TagInput({ label, values, onChange, placeholder, variant }: {
  ...
  placeholder: string; variant?: "alert";
}) {
  ...
  return (
    <div className="mb-4">
      <label className={`block text-xs font-semibold mb-1.5 ${variant === "alert" ? "text-red-500" : "text-gray-500 dark:text-gray-400"}`}>{label}</label>
      ...
      <div className="flex flex-wrap gap-1.5">
        {values.map(v => (
          <span key={v} className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-xs border font-medium ${
            variant === "alert"
              ? "bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 border-red-200 dark:border-red-800"
              : "bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-600"
          }`}>
            {v}
            <button onClick={() => onChange(values.filter(x => x !== v))} className="hover:text-red-500 transition-colors">
              <X size={10} />
            </button>
          </span>
        ))}
...
export function MemberForm({ mode, initial, onSubmit, onClose }: MemberFormProps) {
  const [data, setData] = useState<Member>(initial ?? EMPTY);
  ...
  const addConditional = () => {
    set("conditionalFoods", [...data.conditionalFoods, { food: "", condition: "" }]);
  };
  ...
  const removeConditional = (i: number) => {
    set("conditionalFoods", data.conditionalFoods.filter((_, idx) => idx !== i));
  };
  ...
  const handleSubmit = async () => {
    if (!data.name.trim()) return alert("名前を入力してください");
    if (!data.id.trim()) {
      data.id = data.name.trim().toLowerCase().replace(/\s+/g, "-") + "-" + Date.now();
    }
    setSaving(true);
    await onSubmit(data);
    setSaving(false);
      ...
      <div className="w-full sm:max-w-lg bg-white dark:bg-gray-800 rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90dvh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 px-5 py-4 flex items-center justify-between rounded-t-3xl sm:rounded-t-2xl">
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">
            {mode === "add" ? "メンバーを追加" : "メンバーを編集"}
          </h2>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="px-5 py-5 space-y-1">
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button onClick={addConditional} className="flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700">
                <Plus size={12} />追加
              </button>
            </div>
            <div className="space-y-2">
              {data.conditionalFoods.map((cf, i) => (
                <div key={i} className="flex gap-2 items-start">
                  <input
                    value={cf.food} onChange={e => updateConditional(i, { ...cf, food: e.target.value })}
                    placeholder="食材"
                  ...
                  />
                  <input
                    value={cf.condition} onChange={e => updateConditional(i, { ...cf, condition: e.target.value })}
                    placeholder="条件（例：天ぷらならOK）"
                  ...
                  />
                  <button onClick={() => removeConditional(i)} className="p-1.5 text-red-400 hover:text-red-600">
                    <Trash2 size={14} />
                  </button>
                </div>
              ))}
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 mb-3">
              <input
                value={rankCategory} onChange={e => setRankCategory(e.target.value)}
                onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addRankCategory())}
                placeholder="カテゴリ名（例：麺類）"
                className="flex-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
              <button onClick={addRankCategory} className="px-3 py-2 rounded-lg bg-violet-100 dark:bg-violet-900/30 text-violet-600 dark:text-violet-400 hover:bg-violet-200 transition-colors text-sm">
                追加
              </button>
            </div>
              ...
              {Object.entries(data.rankings).map(([category, items]) => (
                <div key={category} className="p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => removeRankCategory(category)} className="text-red-400 hover:text-red-600">
                      <Trash2 size={12} />
                    </button>
                  </div>
                  <div className="space-y-1 mb-2">
                    {items.map((item, rank) => (
                      <div key={item} className="flex items-center gap-2">
                        ...
                        <button onClick={() => removeRankItem(category, item)} className="text-red-400 hover:text-red-600">
                          <X size={10} />
                        </button>
                      </div>
                    ))}
                  ...
                  <div className="flex gap-2">
                    <input
                      value={rankItem} onChange={e => setRankItem(e.target.value)}
                      onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addRankItem(category))}
                      placeholder="アイテムを追加"
                      className="flex-1 px-2 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-xs text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                    <button onClick={() => addRankItem(category)} className="px-2 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-600 text-gray-600 dark:text-gray-300 hover:bg-gray-200 transition-colors">
                      <Plus size={12} />
                    </button>
                  </div>
                </div>
          ...
          <div className="mb-4">
            ...
            <textarea
              value={data.memo ?? ""} onChange={e => set("memo", e.target.value)}
              placeholder="自由メモ（例：辛いものが苦手）"
              rows={2}
