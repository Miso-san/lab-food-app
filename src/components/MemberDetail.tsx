...
import { X, Heart, ThumbsDown, AlertTriangle, Info, Trophy } from "lucide-react";
...
function Section({ icon, title, color, children }: {
  ...
  return (
    <div className="mb-5">
      <div className={`flex items-center gap-2 mb-2.5 text-sm font-semibold ${color}`}>
        {icon}
        {title}
      </div>
      {children}
...
function TagList({ items, variant }: { items: string[]; variant: "default" | "bad" | "alert" }) {
  ...
  return (
    <div className="flex flex-wrap gap-1.5">
      {items.map((item) => (
        <span key={item} className={`px-2.5 py-1 rounded-full text-xs border font-medium ${cls}`}>
          {item}
        </span>
      ))}
    </div>
...
export function MemberDetail({ member, onClose }: MemberDetailProps) {
  ...
  return (
    ...
    >
      <div
        className="w-full sm:max-w-lg bg-white dark:bg-gray-800 rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90dvh] overflow-y-auto"
        ...
        <div className="sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 px-5 py-4 flex items-center gap-4 rounded-t-3xl sm:rounded-t-2xl">
          ...
          <div className="flex-1 min-w-0">
            ...
            {member.allergies.length > 0 && (
              <p className="text-xs text-red-500 dark:text-red-400">⚠ アレルギーあり</p>
            )}
          </div>
          <button
            onClick={onClose}
            ...
            <Section icon={<Info size={14} />} title="条件付きでOK" color="text-yellow-600 dark:text-yellow-400">
              <div className="space-y-2">
                {member.conditionalFoods.map((cf, i) => (
                  <div
                    key={i}
                    className="px-3 py-2.5 rounded-xl bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 text-sm"
                  >
